...
def createlist_clicked(): 
    status_lbl.configure(text="Create List button was clicked!")
    CreateList(tk.Toplevel(window))

def updatevideos_clicked(): 
...
create_video_list_btn = tk.Button(window, text="Create Video List", command=createlist_clicked)
create_video_list_btn.grid(row=1, column=1, padx=10, pady=10)

update_videos_btn = tk.Button(window, text="Update Videos", command=updatevideos_clicked)
